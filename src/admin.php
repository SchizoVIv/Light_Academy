...
if($_GET['mode']=='7'){
	...
	$mycss = array();
	$cssreg = '/<link\s+rel="stylesheet"\s+href="([^"]+\.css)"/i';
...
if($_GET['mode']=='8'){
  ...
  // Получаем список JS файлов из папки js
  $jsDirectory = __DIR__ . '/js'; // Путь к папке js
  $jsFiles = glob($jsDirectory . '/*.js');
  foreach ($jsFiles as $jsFile) {
      $jsFileName = basename($jsFile);
      echo '<a class="cssjs" href="admin.php?mode=9&fl=js/' . $jsFileName . '">' . $jsFileName . '</a><p>';
  }
};
